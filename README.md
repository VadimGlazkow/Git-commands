...
* [Синхронизация локального и удалённого репозитория](#синхронизация-локального-и-удалённого-репозитория)
...
* [Слияние веток](#cлияние-веток)
* [Работа с удалённым репозиторием](#работа-с-удалённым-репозиторием)
* [Что такое Fork](#что-такое-fork)

[Командная работа в Git](#командная-работа-в-git)
* [Состояние fast-forward](#состояние-fast-forward)
* [Отключение fast-forward](#отключение-fast-forward)
* [git push и fast-forward](#git-push-и-fast-forward)
* [Feature branch workflow](#feature-branch-workflow)
* [Алгоритм-шпаргалка для создания PR](#алгоритм-шпаргалка-для-создания-pr)
* [Алгоритм-шпаргалка для разрешения конфликтов слияния](#алгоритм-шпаргалка-для-разрешения-конфликтов-слияния)
...
* `git log --graph` — нарисуй ветки с помощью «палочек» и «звёздочек».
...
* `git status --ignored` — покажи текущее состояние репозитория, включая файл `.gitignored`.
...
### **Просмотр изменений**
* `git diff` (от англ. **diff**erence, «отличие», «разница») — сравнивни последнюю закоммиченную версию файла с той, что находится в состоянии `modified` или `untracked`;
* `git diff --staged` — сравнивни последнюю закоммиченную версию файла с той, что находится в состоянии `staged`;
...
* `git clone <URL>` (от англ. clone, «клон», «копия») — склонируй репозиторий с URL (HTTPS или SSH) на мой локальный компьютер.
...
#### **Сравнение веток**
* `git diff <branch_1> <branch_2>` (от англ. difference, «отличие», «разница») — покажи разницу между веткой `branch_1` и `branch_2`, где `branch_1` и `branch_2` — имя ветки или хэш коммита ветки;
* `git diff HEAD~2 HEAD~` — покажи разницу между тем коммитом, который был два коммита назад, и предыдущим.

#### **Удаление веток**
* `git branch -d name` — удали ветку `name`, но только если она была полностью объединена с другой — то есть если две ветки стали (или изначально были) частью одной истории;
* `git branch -D name` — удали ветку `name`, даже если она не объединена с main.
...
* `git merge --no-ff <имя ветки>` — флаг `--no-ff` отключит `fast-forward` при слиянии веток;
* Команда `git config [--global] merge.ff false` отключит `fast-forward` «навсегда» (до тех пор, пока вы не вернёте настройку «как было»).

#### **git push и fast-forward**
* `git push --force` — В этом случае команда `git push --force` просто «выкинет» (удалит) коммит D и запишет в `main@origin` коммиты из main.
